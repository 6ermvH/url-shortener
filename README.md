...
Сервис сокращения ссылок на Go с поддержкой in-memory и PostgreSQL хранилища
...
## Допущения или Будующий функционал

- Вероятность совпадения первых 8 байт SHA-256 для двух разных URL 1/2^64
- Два одинаковых URL дадут одну короткую ссылку
- Клиент получает JSON с оригинальным URL и сам выполняет переход
- Сервис полностью открыт, подходит для внутреннего использования
- Ссылки хранятся бессрочно
- Длина коротких ссылок, их формат определён в `service` слое, что даёт гибкость в изменении сервиса
- Решено было не использовать `cobra + viper` для реализации CLI, для такого маленького проекта это слишком
- Документация типа `swagger` или `openapi` не генерировал, из-за простоты проекта
